...
def generation(in_smiles, substring="[NH3+]", mutations=[1, 2, 3, 4], samples=100):

    mol = Chem.MolFromSmiles(in_smiles)
    if mol == None:
        raise Exception('Invalid starting structure encountered')
    randomized_smile_orderings  = [randomize_smiles(mol) for _ in range(samples)]
    ...
    gen_smiles_lst = []
    for smi in canon_smi_ls:
        if substring in smi:
            gen_smiles_lst.append(smi)
    return gen_smiles_lst

def syba_score(found_smiles, n_to_keep=100, n_to_show=9):

    syba = syba = SybaClassifier()
    syba.fitDefaultScore()
    syba_scores = []
    for item in found_smiles:
        syba_scores.append(syba.predict(smi=item))
    A = np.argsort(syba_scores)
    smi_arranged = [found_smiles[i] for i in A]
    scores_arranged = [syba_scores[i] for i in A]
    smi_arranged = smi_arranged[-n_to_keep:]
    smi_arranged.reverse()
    scores_arranged = scores_arranged[-n_to_keep:]
    scores_arranged.reverse()
    mols_ = [Chem.MolFromSmiles(x) for x in smi_arranged]
    img = Draw.MolsToGridImage(mols_[:n_to_show],molsPerRow=n_to_show//3,subImgSize=(400,200))    
    return smi_arranged, scores_arranged, img
